...
{
    public class TextEncoder2Tests
    {
        // Globalny znak ucieczki, który może być zmieniony w jednym miejscu, wpływając na wszystkie testy
        private readonly char _globalEscapingCharacter = ';';

        [Fact]
        // Testuje, czy metoda Encode poprawnie koduje specjalne znaki w tekście, w tym podwajając znak ucieczki.
        public void Encode_WhenTextContainsSpecialCharacters_EncodesCorrectly()
        {
            // Arrange
            var characterPairs = new Dictionary<char, char> { { '>', 'g' }, { '<', 'l' }, { '&', 'a' } };
            var encoder = new TextEncoder2(_globalEscapingCharacter, characterPairs);
        ...
        [Fact]
        // Testuje, czy metoda Decode rzuca wyjątek, gdy napotka nieprawidłową sekwencję po znaku ucieczki.
        public void Decode_WhenEncodedTextContainsUnknownEscapeSequence_ThrowsArgumentException()
        {
            // Arrange
            var characterPairs = new Dictionary<char, char> { { '>', 'g' }, { '<', 'l' }, { '&', 'a' } };
            var encoder = new TextEncoder2(_globalEscapingCharacter, characterPairs);
            // Dodanie nieprawidłowej sekwencji po znaku ucieczki
        ...
        [Fact]
        // Testuje pełny cykl kodowania i dekodowania, w tym obsługę podwajania znaku ucieczki.
